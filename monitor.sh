...
function generate_report {
	...
	if [ $filecount -ge $MAXIMUM_REPORTS ] ; then
		oldfile=`ls ./reports_dir | sort | head -1`
		rm ./reports_dir/$oldfile
	fi
...
function calculate_cpu_usage {
	...
	oldutime=`awk '{ print $14 }' $statfile`
	oldstime=`awk '{ print $15 }' $statfile`
	...
	newutime=`awk '{ print $14 }' $statfile`
	newstime=`awk '{ print $15 }' $statfile`
...
function notify
{
	...
	if [ $cpu_usage_int -gt $CPU_THRESHOLD ]; then
	    /usr/bin/mailx -s "CPU Threshold Exceed" $USER < ./reports_dir/$file_name
	fi
	#Check if process exceeded its CPU or MEM thresholds. If that is the case, send an email to $USER containing the last report

	if [ $cpu_usage_int -gt $CPU_THRESHOLD -o $mem_usage -gt $MEMORY_THRESHOLD ]; then
	    /usr/bin/mailx -s "CPU or MEMORY Threshold Exceed" $USER < ./reports_dir/$file_name
	fi
}
